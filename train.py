...
def main(args):
    ...
    print(f'myKD_method  = {args.myKD_method}')
    print(f'KD_to_copy   = {args.KD_to_copy}')
    # set seed
            ...
            with tqdm(total=len(dataloader)) as pbar:
                for example_num, (x, targets) in enumerate(dataloader):
                    ...
                    if args.teacher_model is not None:
                        ...
                        KD_outputs, KD_hard_loss ,KD_loss ,KD_soft_loss = utils.KD_compute_loss(student_KD,teacher_model, x, targets, My_criterion,alpha=nograd_alpha,compute_grad=True,myKD_method=args.myKD_method)
                        KD_optimizer.step()
                        # temp=copy.deepcopy(nograd_alpha)
                        # for i in range(len(temp)):
                        #     if temp[i]+0.1>1:
                        #         temp[i]=1
                        copy_optimizer.zero_grad()
                        _,_,_,_ = utils.KD_compute_loss(student_copy,teacher_model, x, targets, My_criterion,alpha=1,compute_grad=True,myKD_method=args.myKD_method)
                        copy_optimizer.step()

                        copy2_optimizer.zero_grad()
                        _,_,_,_ = utils.KD_compute_loss(student_copy2,teacher_model, x, targets, My_criterion,alpha=0,compute_grad=True,myKD_method=args.myKD_method)
                        ...
                        PG_loss=utils.RL_compute_loss(alpha,rewards,nn.MSELoss())
                        PG_optimizer.step()
                        # print info
